...
# ==========================================================
# RECENT / LATEST RECORD ORDERING
# ==========================================================

DYNAMIC_RECENT_RECORD_TERMS = {
    "ultimo",
    "ultimi",
    "ultima",
    "ultime",
...
DYNAMIC_TEMPORAL_DURATION_UNITS = {
    "anno",
    "anni",
...
def _dynamic_user_requests_recent_records(
    user_message: str,
) -> bool:

    """
    Determina se l'utente sta chiedendo i record più recenti.
    ...
    Gli ultimi casi rappresentano una DURATA TEMPORALE,
    non una richiesta di ordinamento dei record.
    """
    ...
    # ======================================================
    # ULTIMI / ULTIME + EVENTUALE QUANTITÀ
    # ======================================================
    ...
    for _quantity, following_word in matches:
        ...
        # "ultime vendite"
        #
        # vengono considerati richieste di record recenti.

        return True

    return False

def _dynamic_user_explicitly_requests_ordering(
    user_message: str,
) -> bool:

    """
    Determina se l'utente ha specificato esplicitamente
    come vuole ordinare i risultati.

    In questo caso NON sovrascriviamo l'ordinamento con
    tree_view.default_order.
    """
    ...
    explicit_patterns = [
        ...
        r"\bordine\s+crescente\b",

        r"\bordine\s+decrescente\b",
...
def _dynamic_normalize_tree_default_order(
    default_order: Any,
    fields: Dict[str, Dict[str, Any]],
) -> Optional[str]:

    """
    Normalizza il default_order proveniente direttamente dalla
    tree/list view Odoo.

    Il valore viene considerato autorevole, ma verifichiamo
    comunque che i campi esistano nei metadata reali.
    """
    ...
    for raw_part in default_order.split(","):
        ...
        # --------------------------------------------------
        # Il default_order proviene da Odoo.
        #
        # Verifichiamo comunque che il campo sia presente
        # nei metadata caricati.
        # --------------------------------------------------

        if field_name not in fields:

            print(
                "\n===== TREE VIEW DEFAULT ORDER FIELD SKIPPED ====="
            )
...
def _dynamic_apply_tree_view_default_order(
    params: Dict[str, Any],
    fields: Dict[str, Dict[str, Any]],
    tree_view: Optional[Dict[str, Any]],
    user_message: str,
) -> Dict[str, Any]:

    """
    Applica il default_order della tree/list view Odoo quando
    l'utente chiede i record più recenti e non specifica
    autonomamente un ordinamento.

    Nessun campo data viene inventato.
    """
    ...
    if not _dynamic_user_requests_recent_records(
        user_message
    ):
    ...
    # ======================================================
    # ORDINAMENTO ESPLICITO DELL'UTENTE
    # ======================================================

    if _dynamic_user_explicitly_requests_ordering(
        user_message
    ):

        print(
            "\n===== TREE VIEW DEFAULT ORDER SKIPPED ====="
        )

        print(
            "REASON:",
            "user explicitly requested an ordering"
        )
    ...
    # ======================================================
    # DEFAULT ORDER DELLA TREE VIEW
    # ======================================================

    default_order = tree_view.get(
        "default_order"
    )

    normalized_default_order = (
        _dynamic_normalize_tree_default_order(
    ...
    if normalized_default_order:

        original_order = params.get(
            "order"
        )
        ...
        print(
            "\n===== TREE VIEW DEFAULT ORDER APPLIED ====="
        )
    ...
    # ======================================================
    # NESSUN DEFAULT ORDER DISPONIBILE
    # ======================================================
    #
    # Non inventiamo:
    #
    ...
    # write_date desc
    #
    # Rimuoviamo eventuali ordinamenti generati dal Planner
    # perché non sono stati richiesti esplicitamente dall'utente.
    # ======================================================

    print(
        "\n===== TREE VIEW DEFAULT ORDER UNAVAILABLE ====="
    )

    print(
        "REASON:",
        "recent-record request but tree view has no valid default_order"
    )
...
# ==========================================================
# DYNAMIC RELATIONAL VALUE RESOLUTION
# ==========================================================

def _dynamic_relation_field_metadata(
    field_name: str,
    fields: Dict[str, Dict[str, Any]],
    ...
    # ------------------------------------------------------
    # Supportiamo soltanto il root field.
    #
    # Esempio:
    #
    # workcenter_id
    ...
    # workcenter_id.name
    #
    # viene lasciato al normale domain resolver Odoo.
    # ------------------------------------------------------

    root_field = field_name.split(
        ".",
        1
    )[0].strip()
...
def _dynamic_parse_name_search_results(
    response: Any,
) -> List[Tuple[int, str]]:
...
def _dynamic_normalize_relation_label(
    value: Any,
) -> str:
...
async def _dynamic_name_search_relation(
    odoo_client,
    relation_model: str,
    value: Any,
) -> int:
    ...
    """
    Risolve un valore testuale verso un ID del modello
    relazionato utilizzando esclusivamente l'API generica
    `name_search` di Odoo.
    ...
    La logica di ricerca viene demandata a Odoo stesso.
    """
    ...
    if not isinstance(
        ...
        raise ValueError(
            "Modello relazionale non valido."
        )

    relation_model = (
        relation_model.strip()
    )

    if not relation_model:

        raise ValueError(
            "Modello relazionale vuoto."
        )
    ...
    try:
        ...
        print(
            "\n===== DYNAMIC NAME SEARCH EXACT ERROR ====="
        )
    ...
    exact_matches = (
        _dynamic_parse_name_search_results(
            exact_response
        )
    )
    ...
    if len(
        ...
        print(
            "\n===== DYNAMIC RELATIONAL VALUE RESOLVED ====="
        )
    ...
    if len(
        ...
        normalized_input = (
            _dynamic_normalize_relation_label(
                text_value
            )
        )
        ...
        print(
            "\n===== DYNAMIC RELATIONAL VALUE AMBIGUOUS ====="
        )
        ...
        raise ValueError(
            ...
            f"'{text_value}' produce più corrispondenze "
            f"nel modello '{relation_model}'."
        )
    ...
    try:
        ...
        print(
            "\n===== DYNAMIC NAME SEARCH FUZZY ERROR ====="
        )
    ...
    if not fuzzy_matches:
        ...
        print(
            "\n===== DYNAMIC RELATIONAL VALUE NOT FOUND ====="
        )
        ...
        raise ValueError(
            "Il valore relazionale "
            f"'{text_value}' non è stato trovato "
            f"nel modello '{relation_model}'."
        )
    ...
    raise ValueError(
        "Il valore relazionale "
        f"'{text_value}' è ambiguo nel modello "
        f"'{relation_model}'."
    )


async def _dynamic_resolve_relation_domain_value(
    odoo_client,
    field_name: str,
    operator: str,
    value: Any,
    ...
    field_meta = (
        _dynamic_relation_field_metadata(
            field_name=field_name,
            fields=fields,
        )
    )
    ...
    if operator in {
        ...
        if not isinstance(
            ...
            raise ValueError(
                "Un domain relazionale con operatore "
                f"'{operator}' richiede una lista di valori."
            )
        ...
        for item in value:
            ...
            if not isinstance(
                ...
                raise ValueError(
                    "Valore relazionale non supportato: "
                    f"{item!r}"
                )
    ...
    # not ilike / not like
    #
    # trasformiamo il confronto testuale in un confronto
    # sugli ID trovati dal name_search.
    # ======================================================
    ...
    if operator in {
        ...
        try:
            ...
            raise ValueError(
                "Impossibile risolvere il valore "
                f"'{value}' sul modello relazionale "
                f"'{relation_model}': {exc}"
            )
        ...
        ids = [
            record_id
            for record_id, _label in matches
        ]
...
async def _dynamic_resolve_relation_domain_values(
    *,
    odoo_client,
    params: Dict[str, Any],
    fields: Dict[str, Dict[str, Any]],
    ...
    async def resolve_node(
        ...
        if isinstance(
            ...
            if (
                ...
                field_name = node[0].strip()

                operator = node[1].strip()

                # ----------------------------------------------
                # Se è un path relazionale:
                #
                # workcenter_id.name
                #
                # non usiamo name_search sul root field,
                # perché Odoo può valutare direttamente il
                # path.
                # ----------------------------------------------
                ...
                # --------------------------------------------------
                # Evitiamo di toccare valori già ID-based.
                # --------------------------------------------------
                ...
                resolved_operator, resolved_value = (
                    await _dynamic_resolve_relation_domain_value(
                        odoo_client=odoo_client,
                ...
                print(
                    "\n===== DYNAMIC RELATIONAL DOMAIN RESOLUTION ====="
                )
            ...
            # --------------------------------------------------
            # Dominio annidato / operatori logici
            # --------------------------------------------------
...
async def odoo_agent(
    ...
    # ======================================================
    #
    # La richiesta corrente presente nello state è la fonte
    # autorevole.
    #
    # `messages[-1]` rimane soltanto un fallback legacy.
    # Questo evita che una precedente risposta dell'assistente
    # venga interpretata come richiesta corrente.
    # ======================================================
    ...
    if not current_user_message:
        ...
        raise ValueError(
            "Odoo Agent: richiesta utente corrente mancante."
        )
    ...
    # ======================================================
    #
    # "ultimi ordini", "ultimi record", "più recenti", ecc.
    #
    # usa il default_order reale della tree/list view Odoo.
    #
    # Nessun fallback hardcoded a:
    #
    # id desc
    # create_date desc
    # write_date desc
    ...
    if method == "search_read":
        ...
        params = (
            _dynamic_apply_tree_view_default_order(
                params=params,
    ...
    # name_search("M2")
    #
    # Nessun mapping business hardcoded.
    # ======================================================

    params = await (
        _dynamic_resolve_relation_domain_values(
            odoo_client=odoo_client,
